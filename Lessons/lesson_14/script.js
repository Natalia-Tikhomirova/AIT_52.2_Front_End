...
function createAccount() {
  ...
  if (name) {
    bank.push({
      ...bankAccount, // скопировать всё содержимое из bankAccount и вставить на место точек
...
function showAccounts() {
  ...
  // Проходим по всем аккаунтам и выводим информацию
  for (let i = 0; i < bank.length; i++) {
    const account = bank[i];
    console.log(`Account number: ${account.accountNumber}, Name: ${account.accountHolderName}, Balance: ${account.balance}`);      
  }
}
